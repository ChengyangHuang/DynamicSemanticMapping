...
public:
    ...
    /*
     * Find nearest neighbor's distance and v
     */
    std::pair<float, ElemType> nearestNeighbor(const Point<N>& key) const;
...
template <std::size_t N, typename ElemType> 
std::pair<float, ElemType> KDTree<N, ElemType>::nearestNeighbor(const Point<N>& key) const {
    BoundedPQueue<ElemType> pQueue(1); // BPQ with maximum size k
    if (empty()) {
        // default return value if KD-tree is empty
        std::cout << "tree is empty\n";
        return std::pair<float, ElemType>(std::numeric_limits<float>::infinity(), ElemType()); 
    }

    // Recursively search the KD-tree with pruning
    nearestNeighborRecurse(root_, key, pQueue);

    // return distance and node index
    return std::pair<float, ElemType>(pQueue.best(), pQueue.dequeueMin());
}
